...
import { type Container, promiseCache } from "@brushy/di-core";
import { useRef } from "react";
import { useDIContainer } from "./context";

const isThenable = (value: unknown): value is Promise<unknown> =>
  !!value && (value instanceof Promise || typeof (value as Promise<unknown>).then === "function");
...
const createPromiseProxy = <T>(service: T, tokenKey: string, shouldCache: boolean): T => {
...
export function useInject<T>(token: InjectionToken<T>, options?: InjectOptions): T;
...
export function useInject<T>(token: Token, options?: InjectOptions): T {
  ...
  const stale =
    ref.current === null || ref.current.container !== container || ref.current.token !== token;
