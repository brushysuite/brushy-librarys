...
interface InjectAPI {
  setGlobalContainer(container: Container, options?: { autoCleanRequestScope?: boolean }): void;
...
export const inject: InjectAPI = {
  ...
  getGlobalContainer: (): Container => {
    if (!containerRegistry.hasDefaultContainer()) {
      throw new Error("No global container defined. Use inject.setGlobalContainer() first.");
    }
